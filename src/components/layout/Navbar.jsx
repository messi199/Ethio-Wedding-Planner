    ...
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-lg py-1 z-[60] text-gray-800">
          ...
          <button
            onClick={logout} // Update this line
...
export default function Navbar({ logo, isTransparent = false, user }) {
  ...
  const navbarClasses = `${baseClasses} ${
    isTransparent && !isScrolled ? transparentClasses : solidClasses
  }`;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-baseline space-x-6">
            <Link to="/vendors" className="hover:text-rose-500 font-medium">
              Vendors
            </Link>
            <Link to="/about" className="hover:text-rose-500 font-medium">
              About
            </Link>
            {/* Your smart group link can go here */}
            <Link to="/group" className="hover:text-rose-500 font-medium">
              Group
            </Link>
            <Link to="/contact" className="hover:text-rose-500 font-medium">
              Contact Us
            </Link>
          </div>
          ...
          <div className="px-4 pt-2 pb-4 space-y-3">
            ...
            {user && (
              <div className="border-t border-gray-200 pt-4">
                ...
                <Link
                  to={`/dashboard/${user.accountType}`}
                  className="block mt-2 px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-rose-500 hover:bg-gray-50"
                >
                  My Dashboard
                </Link>
